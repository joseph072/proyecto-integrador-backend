...
module.exports = {
  ...
  obtenerPorCliente(nombre) {
    return new Promise((resolve, reject) => {
      conexion.query(`select ventas.id, ventas.total, clientes.nombre, clientes.direccion FROM ventas inner join clientes on ventas.id_cliente = clientes.id where clientes.nombre = ?`,
          [nombre],
          (err, resultados) => {
          if (err) reject(err);
          else resolve(resultados);
